...
class Coin {
    ...
    public static function create($collection_id, $country_id, $type_id, $rarity_id, $series_id, $material_id, $unit_id, $name, $code, $year, $denomination, $mintage, $description) {
        $db = getDbConnection();
        $stmt = $db->prepare("INSERT INTO coins (collection_id, country_id, type_id, rarity_id, series_id, material_id, unit_id, name, code, year, denomination, mintage, description) 
    ...
    public static function findAllByCollection($collection_id) {
        $db = getDbConnection();
        $stmt = $db->prepare("SELECT * FROM coins WHERE collection_id = :collection_id");
        $stmt->bindParam(':collection_id', $collection_id);
        $stmt->execute();
    ...
    public static function update($id, $collection_id, $country_id, $type_id, $rarity_id, $series_id, $material_id, $unit_id, $name, $code, $year, $denomination, $mintage, $description) {
        $db = getDbConnection();
        $stmt = $db->prepare("UPDATE coins SET collection_id = :collection_id, country_id = :country_id, type_id = :type_id, rarity_id = :rarity_id, series_id = :series_id, material_id = :material_id, unit_id = :unit_id, name = :name, code = :code, year = :year, denomination = :denomination, mintage = :mintage, description = :description WHERE id = :id");
        $stmt->bindParam(':id', $id);
        $stmt->bindParam(':collection_id', $collection_id);
        $stmt->bindParam(':country_id', $country_id);
        $stmt->bindParam(':type_id', $type_id);
    ...
    public static function delete($id) {
        $db = getDbConnection();
        $stmt = $db->prepare("DELETE FROM coins WHERE id = :id");
        $stmt->bindParam(':id', $id);
        $stmt->execute();
    }
}
